...
public class Test1 {
    public static void main(String[] args) throws Exception {
        ...
        OutputStream out = new FileOutputStream("mp3\\lxy.mp3");
        //用缓冲区读写文件
        ...
        while ((len = in.read(buff)) != -1) {//判断是否读到文件末尾
            out.write(buff, 0, len);//从第一个字节开始，向文件写入len字节
        }
        long endtime = System.currentTimeMillis();
        System.out.println("拷贝文件所消耗的时间为：" + (endtime - begintime) + "ms");//拷贝文件所消耗的时间为：23ms
        in.close();
        out.close();
    }
}
